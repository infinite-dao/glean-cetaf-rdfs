...
awk_patternsplit_commands=/opt/jena-fuseki/import-sandbox/bin/patternsplit.awk # NOTE: modify patternsplit.awk to match a beginning triple-data-set
cd "$PWD"

ls_search_pattern='Thread-*tun.fi_2022[0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9]*.normalized.ttl.trig.gz'
IFS=$'\n';
...
for f in `ls $ls_search_pattern | sort --version-sort `; do
  ...
  prefix_normalized_file=`echo "$f" | sed -r 's@.*(Thread-.*tun.fi.*)_modified.rdf.+@\1@'`  ;
