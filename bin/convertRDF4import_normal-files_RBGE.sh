...
echo -e  "############  RBG Edinburgh (RBGE): Parse and normalize RDF to TriG (${RED}debug mode${NOFORMAT}) ####"
else
...
# check https://github.com/infinite-dao/glean-cetaf-rdfs/issues/11
  exit_code=0;
  this_file_issue_11=$( grep --max-count=1 --only-matching --files-with-matches --extended-regexp ':description> +"[^<>"]* +\?>"' "${import_ttl_normalized_trig}") \
    || exit_code=$?
    
  if ! [[ -z "${this_file_issue_11-}" ]];
  then
    echo -en "# ${GREEN}fix https://github.com/infinite-dao/glean-cetaf-rdfs/issues/11 ...${NOFORMAT}\n"
    sed --regexp-extended --in-place 's@(:description>) +"([^<>"]*)  [?]>"@\1 "\2"@g' "${import_ttl_normalized_trig}"
  fi
