...
echo -e  "############  BGBM Berlin: Parse and normalize RDF to TriG (${RED}debug mode${NOFORMAT}) ####"
else
echo -e  "############  BGBM Berlin: Parse and normalize RDF to TriG #################"
fi
...
# plus trig format
  ...
  echo -e  "# ${GREEN}(4)   add/check ROR ID (... herbarium.bgbm.org aso.)${NOFORMAT}" ;
  sed --regexp-extended --in-place '
  # ## ROR_OR_INSTITUTION of herbarium.bgbm.org/object/ --- https://ror.org/00bv4cx53
  ...
  # add ROR_OR_INSTITUTION ID eventually to the final dot, and remove possible duplicates
      ...
      s@<http://rs.tdwg.org/dwc/terms/institutionID>  <https://ror.org/00bv4cx53>\s+[;]\n +(<.+)(<http://rs.tdwg.org/dwc/terms/institutionID>  https://ror.org/00bv4cx53 .)@\1\2@; 
  ...
  } ## end ROR_OR_INSTITUTION herbarium.bgbm.org/object/
