...
echo -e  "############  Naturalis: Parse and normalize RDF to TriG (${RED}debug mode${NOFORMAT}) ####"
else
echo -e  "############  Naturalis: Parse and normalize RDF to TriG #################"
fi
...
# plus trig format
  ...
  echo -e  "# ${GREEN}(4)   add/check ROR ID; Botany Pilot modifications ... (data.biodiversitydata.nl)${NOFORMAT}" ;
  sed --in-place --regexp-extended '
# ## ROR_OR_INSTITUTION of data.biodiversitydata.nl/naturalis/specimen/ --- https://ror.org/0566bfb96
/^<https?:\/\/data.biodiversitydata.nl\/naturalis\/specimen\/[^<>/]+>/ {
  ...
  # add ROR_OR_INSTITUTION ID eventually to the final dot, and remove possible duplicates
     ...
     s@<http://rs.tdwg.org/dwc/terms/institutionID>  <https://ror.org/0566bfb96>\s+[;]\n +(<.+)(<http://rs.tdwg.org/dwc/terms/institutionID>  https://ror.org/0566bfb96 .)@\1\2@; 
...
} ## end ROR_OR_INSTITUTION data.biodiversitydata.nl/naturalis/specimen/
