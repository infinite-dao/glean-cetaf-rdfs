...
echo -e  "############  Paris: Parse and normalize RDF to TriG (${RED}debug mode${NOFORMAT}) ####"
else
echo -e  "############  Paris: Parse and normalize RDF to TriG #################"
fi
...
# plus trig format
  ...
  echo -e  "# ${GREEN}(4)   add/check ROR ID; Botany Pilot modifications ... (coldb.mnhn.fr)${NOFORMAT}" ;
...
} ## end ROR_OR_INSTITUTION coldb.mnhn.fr/catalognumber/mnhn/
...
echo  -e "# ${GREEN}Added collection URL ${BLUE_BOLD}dcterms:isPartOf <http://coldb.mnhn.fr/catalognumber/mnhn/pc/>${GREEN}, eg. /pc/ for cryptogame plants aso.${NOFORMAT}"
