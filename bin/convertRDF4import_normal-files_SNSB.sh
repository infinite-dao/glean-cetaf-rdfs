...
echo -e  "############  SNSB, Munich: Parse and normalize RDF to TriG (${RED}debug mode${NOFORMAT}) ####"
else
echo -e  "############  SNSB, Munich: Parse and normalize RDF to TriG #################"
fi
...
# plus trig format
  ...
  echo -e  "# ${GREEN}(4)   add/check ROR ID (... id.snsb.info…/collection/ or …/collection_monitoring/ aso.)${NOFORMAT}" ;
  sed --regexp-extended --in-place '
  # ## ROR_OR_INSTITUTION of id.snsb.info/snsb/collection/ --- https://ror.org/05th1v540
  /^<https?:\/\/id.snsb.info\/snsb\/collection\/[0-9]+\/[^<>]+>/ {
    :label_uri-entry_id.snsb.infoSLASHsnsbSLASHcollectionSLASH
  N                                     # append lines via \n into patternspace
    / \.$/!b label_uri-entry_id.snsb.infoSLASHsnsbSLASHcollectionSLASH # go back if last char is not a dot
  # add ROR_OR_INSTITUTION ID eventually to the final dot, and remove possible duplicates
      ...
      s@<http://rs.tdwg.org/dwc/terms/institutionID>  <https://ror.org/05th1v540>\s+[;]\n +(<.+)(<http://rs.tdwg.org/dwc/terms/institutionID>  https://ror.org/05th1v540 .)@\1\2@; 
  ...
  } ## end ROR_OR_INSTITUTION id.snsb.info/snsb/collection/
  # ## ROR_OR_INSTITUTION of id.snsb.info/snsb/collection_monitoring/ --- https://ror.org/05th1v540
  /^<https?:\/\/id.snsb.info\/snsb\/collection_monitoring\/[0-9]+\/[^<>]+>/ {
    :label_uri-entry_id.snsb.infoSLASHsnsbSLASHcollection_monitoringSLASH
    N                                     # append lines via \n into patternspace
    / \.$/!b label_uri-entry_id.snsb.infoSLASHsnsbSLASHcollection_monitoringSLASH # go back if last char is not a dot
    # add ROR_OR_INSTITUTION ID eventually to the final dot, and remove possible duplicates
      s@(\s+[.])$@ ;\n        <http://rs.tdwg.org/dwc/terms/institutionID>  <https://ror.org/05th1v540>\1@;
      s@<http://rs.tdwg.org/dwc/terms/institutionID>  <https://ror.org/05th1v540>\s+[;]\n +(<.+)(<http://rs.tdwg.org/dwc/terms/institutionID>  https://ror.org/05th1v540 .)@\1\2@; 
    # add dcterms:isPartOf, dcterms:hasPart, dcterms:conformsTo
    ...
    s@(\n +<http://rs.tdwg.org/dwc/iri/recordedBy>  <http://www.wikidata.org/entity/[^<>]+>\s+[;.])(\n +<.+[.])$@\n        <http://purl.org/dc/terms/hasPart>  <http://www.wikidata.org/entity/> ;\1\2@;
    s@(\n +<http://rs.tdwg.org/dwc/iri/recordedBy>  <http://viaf.org/viaf/[^<>]+>\s+[;.])(\n +<.+[.])$@\n        <http://purl.org/dc/terms/hasPart>  <http://viaf.org/viaf/> ;\1\2@;
  } ## end ROR_OR_INSTITUTION id.snsb.info/snsb/collection_monitoring/
